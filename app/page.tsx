...
export default function Home() {
  ...
  /* helper: is inside a sub-flow (not on tabs) */
  ...
  const Sidebar = () => (
    <div className="hidden lg:flex w-[52px] h-screen bg-navy flex-col items-center py-4 gap-0.5 flex-shrink-0">
      ...
      <SbIcon icon={I.home(true)} active />
  ...
  const SbIcon = ({ icon, active, badge }: { icon: React.ReactNode; active?: boolean; badge?: number }) => (
    <button className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all relative ${active ? "bg-white/15 text-white" : "text-white/40 hover:text-white/70 hover:bg-white/5"}`}>
      {icon}
      {badge ? <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-danger-700 text-white text-[8px] font-bold rounded-full flex items-center justify-center" style={{ animation: "fadeIn .3s ease" }}>{badge}</span> : null}
    </button>
  );
    ...
    <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-nn-100 ${inSubFlow ? "hidden" : ""}`}>
      <div className="flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom)] h-14">
        {([
          ...
          <button
            key={tab.key}
            onClick={() => { setMobileTab(tab.key); if (tab.key === "home") setScreen("dashboard"); }}
  ...
  const LeftPanel = () => (
    <div className="hidden lg:flex w-[264px] h-screen border-r border-nn-100 bg-white flex-col flex-shrink-0">
      <div className="px-4 pt-4 pb-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="w-8 h-[18px] bg-sea-900 rounded-full relative cursor-pointer"><div className="w-3 h-3 bg-white rounded-full absolute right-[3px] top-[3px]" /></div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <PanelEvItem name="William Smith" project="Ozempic Launch London" time="09:34 AM" selected={selectedPanel === "william"} onClick={startReview} />
        <PanelEvItem name="Rosa Claramunt" project="Ozempic Event Madrid" time="09:34 AM" />
      </div>
    </div>
  );

  const PanelEvItem = ({ name, project, time, selected, onClick }: { name: string; project: string; time: string; selected?: boolean; onClick?: () => void }) => (
    <button onClick={onClick} className={`w-full text-left px-4 py-3 border-b border-nn-100 transition-colors ${selected ? "bg-sea-50 border-l-2 border-l-sea-900" : "hover:bg-nn-50 active:bg-nn-100"}`}>
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-[13px] font-medium text-nn-900">{name}</span>
        <span className="text-[11px] text-nn-400">{time}</span>
      </div>
      <div className="text-[12px] text-nn-700">{project}</div>
    ...
    <div className="flex-1 h-full overflow-y-auto bg-nn-50 pb-20" style={{ animation: "fadeIn .2s ease" }}>
      <div className="bg-white px-4 pt-5 pb-3 border-b border-nn-100">
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[12px] text-nn-500">Unreviewed</span>
          <div className="w-8 h-[18px] bg-sea-900 rounded-full relative"><div className="w-3 h-3 bg-white rounded-full absolute right-[3px] top-[3px]" /></div>
        </div>
      ...
      <div className="p-4 space-y-2">
        {[
          { name: "William Smith", project: "Ozempic Launch London", time: "09:34 AM", action: startReview },
          { name: "Rosa Claramunt", project: "Ozempic Event Madrid", time: "09:34 AM" },
        ].map((item) => (
          <button key={item.name} onClick={item.action} className="w-full text-left bg-white rounded-xl p-4 shadow-[0_1px_3px_rgba(0,25,101,0.04)] active:bg-nn-50 transition-colors">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[14px] font-semibold text-nn-900">{item.name}</span>
              <span className="text-[12px] text-nn-400">{item.time}</span>
            </div>
            <div className="text-[13px] text-nn-700">{item.project}</div>
            <div className="flex items-center justify-between mt-2">
              <span className="text-[12px] text-nn-400">5 evidences in queue</span>
              <span className="text-nn-400">{I.chevron}</span>
            </div>
          </button>
        ))}
      </div>
    ...
    <div className="flex-1 h-full overflow-y-auto bg-nn-50 px-4 pt-5 pb-20 lg:p-6 lg:pb-6" style={{ animation: "fadeIn .2s ease" }}>
      <div className="lg:max-w-[1080px]">
        ...
        <div className="lg:hidden mb-4">
          <p className="text-[14px] text-nn-500">Welcome back, <span className="font-semibold text-navy">William</span></p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 lg:gap-3 mb-5 lg:mb-6">
          ...
          <div className="bg-white rounded-xl p-3.5 lg:p-4 shadow-[0_1px_3px_rgba(0,25,101,0.04)]">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] lg:text-[12px] text-nn-500 font-medium">New evidences</span>
              <span className="text-danger-700 text-[10px] lg:text-[11px] font-medium">↘ -20%</span>
            </div>
            ...
            <button onClick={startReview} className="px-3 lg:px-3.5 py-1.5 bg-sea-900 text-white text-[11px] font-medium rounded-full active:bg-sea-800 hover:bg-sea-800 transition-colors flex items-center gap-1.5">
              <span className="w-1 h-1 bg-white/60 rounded-full" /> Review now
            </button>
          </div>
          {/* Active projects */}
          <div className="bg-white rounded-xl p-3.5 lg:p-4 shadow-[0_1px_3px_rgba(0,25,101,0.04)]">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] lg:text-[12px] text-nn-500 font-medium">Active projects</span>
              <span className="text-ocean-700 text-[10px] lg:text-[11px] font-medium">↗ +12.5%</span>
            </div>
          ...
          <div className="bg-white rounded-xl p-3.5 lg:p-4 shadow-[0_1px_3px_rgba(0,25,101,0.04)]">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] lg:text-[12px] text-nn-500 font-medium">Evidences received</span>
              <span className="text-danger-700 text-[10px] lg:text-[11px] font-medium">↘ -20%</span>
            </div>
          ...
          <div className={`bg-white rounded-xl p-3.5 lg:p-4 relative overflow-hidden ${alertVisible ? "shadow-[0_1px_6px_rgba(219,58,31,0.1)]" : "shadow-[0_1px_3px_rgba(0,25,101,0.04)]"}`}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] lg:text-[12px] text-nn-500 font-medium">Alerts</span>
              <span className="text-ocean-700 text-[10px] lg:text-[11px] font-medium">↗ +4.5%</span>
            </div>
        ...
        <div className="lg:hidden mb-3">
          <input type="text" placeholder="Filter tasks..." className="w-full px-3 py-2.5 text-[14px] border border-nn-200 rounded-xl bg-white outline-none focus:border-sea-500 transition-colors text-nn-700 placeholder:text-nn-400" />
        </div>
            ...
            <button onClick={() => setScreen("ticket")} className="w-full text-left bg-danger-50 rounded-xl p-3.5 ring-1 ring-danger-200 active:bg-danger-100 transition-colors" style={{ animation: "slideUp .35s ease" }}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[11px] text-danger-800">TASK-9201</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-danger-700 font-medium">↑ Critical</span>
                </div>
            ...
            <div key={r.id} className="bg-white rounded-xl p-3.5 shadow-[0_1px_3px_rgba(0,25,101,0.04)]">
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-mono text-[11px] text-nn-500">{r.id}</span>
                </div>
        ...
        <div className="hidden lg:block bg-white rounded-xl shadow-[0_1px_3px_rgba(0,25,101,0.04)] overflow-hidden">
          <div className="grid grid-cols-[36px_110px_1fr_110px_90px_32px] px-4 py-2 border-b border-nn-100 text-[11px] text-nn-500 font-medium uppercase tracking-wider">
            <div /><div>Task</div><div>Title</div><div>Status</div><div>Priority</div><div />
          </div>
          {alertVisible && (
            <div
              ...
              onMouseEnter={() => setHoveredAlert(true)} onMouseLeave={() => setHoveredAlert(false)}
            ...
            >
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="ml-0.5 px-1.5 py-0.5 bg-danger-700 text-white text-[9px] font-bold rounded flex items-center gap-0.5">{I.alert} ALCOHOL</span>
              </div>
              ...
              {hoveredAlert && (
                <div className="absolute right-10 top-1/2 -translate-y-1/2 z-10 bg-navy text-white rounded-lg shadow-xl px-3 py-2" style={{ animation: "fadeIn .12s ease" }}>
                  <button onClick={() => setScreen("ticket")} className="text-[11px] font-medium whitespace-nowrap hover:text-sea-300 transition-colors flex items-center gap-1.5">{I.sparkle} Solve with NovoVision Agent</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,25,101,0.08)] p-5 lg:p-7">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-[11px] text-nn-500">Ozempic Launch London — W. Smith</p>
            </div>
          </div>
          <div className="space-y-2.5 mb-5">
            {loadingSteps.map((step, i) => {
              const done = loadingStep > i, active = loadingStep === i;
              return (
          ...
          <div className="h-1 bg-nn-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500 ease-out" style={{ width: `${(loadingStep / loadingSteps.length) * 100}%`, background: loadingDone ? "#419792" : "#005AD2" }} />
          </div>
          {loadingDone && (
    ...
    <div className="flex-1 h-full overflow-y-auto bg-nn-50" style={{ animation: "fadeIn .2s ease" }}>
      ...
      <div className="px-4 pt-4 lg:pt-6 lg:px-6 pb-6 lg:max-w-[760px] lg:mx-auto">
        ...
        <div className="flex items-center justify-between mb-4 lg:mb-5">
          <div>
            <h1 className="hidden lg:block text-[18px] font-bold text-navy">Evidence Review</h1>
            <p className="text-[12px] text-nn-500 mt-0.5">Ozempic Launch London — W. Smith — 5 evidences</p>
          </div>
          <div className="flex items-center gap-1.5 lg:gap-2">
            <span className="px-2 py-0.5 lg:py-1 bg-ocean-50 text-ocean-800 text-[11px] font-medium rounded-full">4 Clear</span>
            <span className="px-2 py-0.5 lg:py-1 bg-danger-50 text-danger-700 text-[11px] font-medium rounded-full flex items-center gap-1">{I.alert} 1 Alert</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {evidences.map((ev, i) => (
            <div
              key={ev.id}
              className={`bg-white rounded-xl p-3 lg:p-3.5 transition-all ${ev.status === "alert" ? "shadow-[0_2px_12px_rgba(219,58,31,0.08)] ring-1 ring-danger-200" : "shadow-[0_1px_3px_rgba(0,25,101,0.04)]"}`}
              style={{ animation: `slideUp .3s ease ${i * 0.06}s both` }}
            >
              {/* Mobile layout: stacked */}
              <div className="flex items-start gap-2.5">
                {ev.status === "ok" ? (
                  <div className="w-7 h-7 bg-ocean-50 rounded-lg flex items-center justify-center text-ocean-700 flex-shrink-0 mt-0.5">{I.check}</div>
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[12px] lg:text-[12px] font-medium text-nn-900 mb-1 leading-snug">{ev.title}</p>
                  <div className="flex items-center justify-between">
                    <div className="text-[11px] text-nn-400 flex items-center gap-2">
                      AI: {ev.confidence}%
                      <span className="inline-block w-10 h-[3px] bg-nn-100 rounded-full overflow-hidden align-middle">
                    ...
                    {ev.status === "alert" ? (
                      <button onClick={() => setScreen("ticket")} className="px-3 py-1.5 lg:px-3.5 lg:py-2 bg-danger-700 text-white text-[11px] lg:text-[12px] font-medium rounded-lg active:bg-danger-800 hover:bg-danger-800 transition-colors flex items-center gap-1">
                        {I.alert} Review
                      </button>
                    ) : (
      ...
      <div className="px-4 pt-4 lg:pt-6 lg:px-6 pb-8 lg:max-w-[880px] lg:mx-auto">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[1fr_320px] gap-4 lg:gap-5">
          ...
          <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(0,25,101,0.06)] overflow-hidden">
            <div className="bg-danger-700 text-white px-4 lg:px-5 py-2.5 flex items-center gap-2">
              {I.alert}
              <span className="font-medium text-[12px]">Alcohol detected in expense ticket</span>
            </div>
            <div className="p-4 lg:p-6">
              <div className="bg-nn-50 rounded-xl border border-nn-100 p-4 lg:p-5 lg:max-w-[360px] lg:mx-auto font-mono text-[11px] lg:text-[12px]">
                <div className="text-center mb-3 pb-3 border-b border-dashed border-nn-300">
                  <div className="font-bold text-[13px] lg:text-[14px] text-navy mb-0.5">RESTAURANTE EL CELLER</div>
                ...
                <div className="space-y-1 mb-3 pb-3 border-b border-dashed border-nn-300 text-nn-800">
                  ...
                  <div className="flex justify-between items-center bg-danger-50 -mx-2 px-2 py-1.5 rounded-lg ring-1 ring-danger-200">
                    <div className="flex items-center gap-1.5"><span className="text-danger-700">{I.wine(14)}</span><span className="text-danger-800 font-bold text-[11px]">2x Rioja Reserva</span></div>
                    <span className="text-danger-800 font-bold">68.00€</span>
                  </div>
                  <div className="flex justify-between items-center bg-danger-50 -mx-2 px-2 py-1.5 rounded-lg ring-1 ring-danger-200">
                    <div className="flex items-center gap-1.5"><span className="text-danger-700">{I.wine(14)}</span><span className="text-danger-800 font-bold text-[11px]">1x Copa Albariño</span></div>
                ...
                <div className="space-y-0.5 text-nn-600">
                  <div className="flex justify-between text-[10px]"><span>Subtotal</span><span>288.00€</span></div>
                  <div className="flex justify-between text-[10px]"><span>IVA (10%)</span><span>28.80€</span></div>
                  <div className="flex justify-between font-bold text-[13px] lg:text-[14px] text-navy pt-1.5 border-t border-nn-300"><span>TOTAL</span><span>316.80€</span></div>
                </div>
              </div>
            </div>
          </div>

          {/* Details + Actions — stacked on mobile */}
          <div className="space-y-3">
            ...
            <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,25,101,0.04)] p-4">
              ...
              <div className="space-y-2.5 text-[12px]">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-nn-400 text-[10px] uppercase tracking-wider">Total alcohol</span>
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-semibold text-nn-700 text-[14px]">99%</p>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-[0_1px_3px_rgba(0,25,101,0.04)] p-4">
              ...
              <div className="space-y-2 text-[12px]">
                {([["Employee", "William Smith"], ["Project", "Ozempic Launch London"], ["Activity", "Dinner with HCPs"], ["Date", "14 Mar 2025"], ["Source", "SAP Concur"]] as const).map(([k, v]) => (
                  <div key={k} className="flex justify-between"><span className="text-nn-500">{k}</span><span className="font-medium text-nn-800">{v}</span></div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {!agentTriggered ? (
                <button onClick={() => setAgentTriggered(true)} className="w-full py-3 lg:py-2.5 bg-navy text-white rounded-xl font-medium text-[13px] lg:text-[12px] active:bg-navy/90 hover:bg-navy/90 transition-all shadow-[0_2px_8px_rgba(0,25,101,0.2)] flex items-center justify-center gap-2">
                  <span className="text-sea-300">{I.sparkle}</span> Solve with NovoVision Agent
                </button>
              ) : (
                <div className="w-full py-3 lg:py-2.5 bg-sea-50 border border-sea-200 text-sea-900 rounded-xl font-medium text-[13px] lg:text-[12px] text-center flex items-center justify-center gap-2" style={{ animation: "fadeIn .25s ease" }}>
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <Sidebar />
      {/* Desktop left panel — only on dashboard */}
